...
## 🚀 Quick Start with Docker Compose[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#-quick-start-with-docker-compose "Direct link to 🚀 Quick Start with Docker Compose")

The fastest way to get started with observability is with the pre-configured Docker Compose:

```
# Spin up Open WebUI and the latest Grafana LGTM stack, all-in-onedocker compose -f docker-compose.otel.yaml up -d
```
...
After startup, access the Grafana dashboard at [http://localhost:3000](http://localhost:3000/) Login: `admin` / `admin`

## ⚙️ Environment Variables[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#%EF%B8%8F-environment-variables "Direct link to ⚙️ Environment Variables")
...
```
  open-webui:    environment:      - ENABLE_OTEL=true      - ENABLE_OTEL_TRACES=true      - ENABLE_OTEL_METRICS=true      - OTEL_EXPORTER_OTLP_INSECURE=true # Use insecure connection for OTLP, you may want to remove this in production      - OTEL_EXPORTER_OTLP_ENDPOINT=http://grafana:4317      - OTEL_SERVICE_NAME=open-webui      # You may set OTEL_BASIC_AUTH_USERNAME/PASSWORD here if needed
```

## 📊 Data Collection[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#-data-collection "Direct link to 📊 Data Collection")

### Distributed Tracing[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#distributed-tracing "Direct link to Distributed Tracing")
...
### Metrics Collection[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#metrics-collection "Direct link to Metrics Collection")
...
## 🔧 Custom Collector Setup[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#-custom-collector-setup "Direct link to 🔧 Custom Collector Setup")

To use a different (external) OpenTelemetry Collector/Stack:

```
docker run -d --name open-webui \  -p 8088:8080 \  -e ENABLE_OTEL=true \  -e ENABLE_OTEL_TRACES=true \  -e ENABLE_OTEL_METRICS=true \  -e OTEL_EXPORTER_OTLP_ENDPOINT=http://your-collector:4317 \  -e OTEL_EXPORTER_OTLP_INSECURE=true \  -e OTEL_SERVICE_NAME=open-webui \  -v open-webui:/app/backend/data \  ghcr.io/open-webui/open-webui:main
```

## 🚨 Troubleshooting[](https://docs.openwebui.com/getting-started/advanced-topics/monitoring/otel#-troubleshooting "Direct link to 🚨 Troubleshooting")
